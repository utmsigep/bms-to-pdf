...
class SubmittedApplication
  def initialize(params)
    @params = params
    csv = params[:csv]
    raise 'No file provided.' unless csv

    raise 'Invalid file format. Must be a CSV.' unless csv[:type] == 'text/csv'
    raise "File is too large. Must be below #{FILE_SIZE_LIMIT_MB} MB." unless csv[:tempfile].size < FILE_SIZE_LIMIT_MB * 125000

    @table = CSV.parse(File.read(csv[:tempfile].path), headers: true, liberal_parsing: true, header_converters: lambda { |h|
      h.strip
    })
  ...
  def to_pdf
    ...
    pdf.font_families.update('Lato' => {
      normal: "#{FONT_DIR}/Lato-Regular.ttf",
      italic: "#{FONT_DIR}/Lato-Italic.ttf",
      bold: "#{FONT_DIR}/Lato-Bold.ttf",
      bold_italic: "#{FONT_DIR}/Lato-BoldItalic.ttf"
    })
    pdf.font 'Lato'
    pdf.fill_color '000000'

    if @params[:show_summary] == "1"
      ...
      submission_dates = @table.by_col['Submission Date'].map! { |sd| Date.parse(sd) }
      ...
      pdf.move_down 10
      y_position = pdf.cursor
      pdf.text_box Date.parse(histogram.keys.min).strftime('%-m/%-d/%Y'),
                   at: [20, y_position], width: 250, height: 20, size: 8, inline_format: true

      pdf.text_box Date.parse(histogram.keys.max).strftime('%-m/%-d/%Y'),
                   at: [270, y_position], width: 270, height: 20, size: 8, align: :right
      ...
      if duplicates.count > 0
        pdf.text "<strong><color rgb='FF0000'>Possible Duplicate Applications</color></strong>",
                 inline_format: true
        pdf.move_down 5
      ...
      pdf.text "<font size='10'><b>File:</b> #{@params[:csv][:filename]}      <b>Generated:</b> #{Time.now.strftime('%-m/%-d/%Y %-I:%M %p')}</font>",
               inline_format: true
    ...
    @table.each_with_index do |row, i|
      ...
      pdf.move_down 10
      unless @params[:hide_identifiers] == "1"
        y_position = pdf.cursor
        pdf.bounding_box [0, y_position], width: 400, height: 40, overflow: :shrink_to_fit do
          pdf.text "<font size='18'><b>#{row['First Name']} #{row['Last Name']}</b> (#{row['Expected Graduation Year']})</font>",
                   inline_format: true
        ...
        pdf.bounding_box [400, y_position], width: 140, height: 40, overflow: :shrink_to_fit do
          unless row['Date of Birth'].empty?
            ...
            age = ((submission_date.to_time - birth_date.to_time) / 31_557_600).floor
      ...
      pdf.text_box "<strong>Current or Intended Major/Minor</strong>\n#{row['Current or Intended Major/Minor']}",
                   at: [0, y_position], width: 280, height: 25, overflow: :shrink_to_fit, inline_format: true

      pdf.text_box "<strong>Cumulative HS or College GPA</strong>\n#{row['Cumulative High School or College GPA']}",
                   at: [280, y_position], width: 160, height: 25, overflow: :shrink_to_fit, inline_format: true

      pdf.text_box "<strong>ACT/SAT Score</strong>\n#{row['ACT/SAT Score']}",
                   at: [440, y_position], width: 100, height: 25, overflow: :shrink_to_fit, inline_format: true
