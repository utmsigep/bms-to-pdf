...
class SubmittedApplication
  ...
  def to_pdf
    ...
    if @upload_params[:show_summary] == "1"
      ...
      pdf.text "<font size='10'><b>File:</b> #{@upload_params[:csv].original_filename}      <b>Generated:</b> #{DateTime.now.in_time_zone('US/Eastern').strftime('%-m/%-d/%Y %-I:%M %p ET')}</font>",
               inline_format: true
      ...
      unless @upload_params[:hide_identifiers] == "1"
        ...
        pdf.bounding_box [0, y_position], width: 400, height: 40, overflow: :shrink_to_fit do
          pdf.text "<font size='18'><b>#{row['First Name']} #{row['Last Name']}</b> (#{row['Expected Graduation Year']})</font>",
                   inline_format: true
          pdf.text "#{row['Email']}    #{row['Cell Phone Number'].gsub(/^(\d{3})(\d{3})(\d{4})$/,
                                                                       '(\1) \2-\3')}"
        end
